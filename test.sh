...
template_id=$(aws ec2 create-launch-template --launch-template-name TheTemplate --launch-template-data '{"ImageId": "$ami_id","InstanceType": "t2.small" , "KeyName": "vockey", "SecurityGroupIds": ["$sg_load_id"], "TagSpecifications":[{"ResourceType":"instance", "Tags": [{"Key":"Proyecto","Value":"TheProyect"}]}]}' --query 'LaunchTemplate.[LaunchTemplateId,LaunchTemplateName]' --output text | awk '{ print $1 }')
